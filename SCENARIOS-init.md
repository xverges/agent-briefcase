...
Each target repo gets a `.briefcase.lock` file that tracks which briefcase commit
was last synced and which files are managed. **Commit this file** — it enables
detection of local modifications, automatic cleanup of removed files, and
staleness checks in CI. The synced content files are gitignored, but
`.briefcase.lock` is not.
